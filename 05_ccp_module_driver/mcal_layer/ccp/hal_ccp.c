...
STD_RETURN_TYPE CCP_Init(const ccp_t *_ccp_obj) {
    ...
    if(NULL == _ccp_obj){
        ...
        RET = PIN_INITIALIZE(&(_ccp_obj->ccp_pin)); // PIN Configurations
        CCP_Interrupt_Config(_ccp_obj); // Interrupt Configurations for CCP1 and CCP2 Modules
...
STD_RETURN_TYPE CCP1_Capture_Mode_Read_Value(uint16 *capture_value) {
    ...
    if(NULL == capture_value) {
        ...
        capture_temp_value.ccpr_low = CCPR1L; // Copy captured value
        capture_temp_value.ccpr_high = CCPR1H;
        
        *capture_value = capture_temp_value.ccpr_16Bit; // Write the 16-bit capture value
        RET = E_OK;
    }
    return RET;
}
...
STD_RETURN_TYPE CCP_IsCompareComplete(uint8 *_compare_status){
    STD_RETURN_TYPE RET = E_NOT_OK;
...
STD_RETURN_TYPE CCP_Compare_Mode_Set_Value(const ccp_t *_ccp_obj, uint16 compare_value){
    ...
    if(NULL == _ccp_obj){
        ...
        capture_temp_value.ccpr_16Bit = compare_value; // Copy compared value
        if(CCP1_INST == _ccp_obj->ccp_inst) {
...
static void CCP_PWM_Mode_Config(const ccp_t *_ccp_obj) {
    PR2 = (uint8)(((float)_XTAL_FREQ / ((float)_ccp_obj->PWM_Frequency * 4.0 * (float)_ccp_obj->timer2_prescaler_value * 
                    (float)_ccp_obj->timer2_postscaler_value)) - 1); // PWM Frequency Initialization
...
static STD_RETURN_TYPE CCP_Capture_Mode_Config(const ccp_t *_ccp_obj) {
    ...
    if(CCP1_INST == _ccp_obj->ccp_inst) { // CCP1 Module Capture variants Mode Initialization
    ...
    else if(CCP2_INST == _ccp_obj->ccp_inst) { // CCP2 Module Capture variants Mode Initialization
        switch(_ccp_obj->ccp_mode_variant){
...
static STD_RETURN_TYPE CCP_Compare_Mode_Config(const ccp_t *_ccp_obj){
    ...
    if(CCP1_INST == _ccp_obj->ccp_inst) {
        switch(_ccp_obj->ccp_mode_variant) { // CCP1 Module Compare variants Mode Initialization
