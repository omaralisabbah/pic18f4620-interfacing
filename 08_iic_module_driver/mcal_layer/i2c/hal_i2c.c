...
STD_RETURN_TYPE MSSP_I2C_Init(const mssp_i2c_t *i2c_obj) {
    ...
    if(NULL == i2c_obj) {
        ...
        MSSP_MODULE_DISABLE_CFG(); // Disable MSSP I2C Module

        if(I2C_MSSP_MASTER_MODE == i2c_obj->i2c_cfg.i2c_mode) { // MSSP I2C Master Mode Configurations
            I2C_Master_Mode_Clock_Configurations(i2c_obj); // Disable MSSP I2C Module I2C Master Mode Clock Configurations
        } else if(I2C_MSSP_SLAVE_MODE == i2c_obj->i2c_cfg.i2c_mode) { // MSSP I2C Slave Mode Configurations
            if(I2C_GENERAL_CALL_ENABLE == i2c_obj->i2c_cfg.i2c_general_call) { // I2C Slave Mode General Call Configurations
                I2C_GENERAL_CALL_ENABLE_CFG();
            } else if(I2C_GENERAL_CALL_DISABLE == i2c_obj->i2c_cfg.i2c_general_call) {
                I2C_GENERAL_CALL_DISABLE_CFG();
            } else { /* Nothing */ }
            
            SSPCON1bits.WCOL = 0; // Clear the Write Collision Detect (No Collision)
            SSPCON1bits.SSPOV = 0; // Clear the Receive Overflow Indicator (No Overflow)
            SSPCON1bits.CKP = 1; // Release the clock
            SSPADD = i2c_obj->i2c_cfg.i2c_slave_address; // Assign the I2C Slave Address
            I2C_Slave_Mode_Configurations(i2c_obj); // I2C Slave Mode Configurations
        } else { /* Nothing */ }

        MSSP_I2C_Mode_GPIO_CFG(); // I2C Master Mode GPIO Configurations
        if(I2C_SLEW_RATE_DISABLE == i2c_obj->i2c_cfg.i2c_slew_rate) { // MSSP I2C Slew Rate Control
            I2C_SLEW_RATE_DISABLE_CFG();
        } else if(I2C_SLEW_RATE_ENABLE == i2c_obj->i2c_cfg.i2c_slew_rate) {
            I2C_SLEW_RATE_ENABLE_CFG();
        } else { /* Nothing */ }
        
        if(I2C_SMBus_ENABLE == i2c_obj->i2c_cfg.i2c_SMBus_control) { // MSSP I2C SMBus Control
...
#endif
        MSSP_MODULE_ENABLE_CFG(); // Enable MSSP I2C Module
        RET = E_OK;
    }
    return RET;
}
...
STD_RETURN_TYPE MSSP_I2C_Master_Send_Start(const mssp_i2c_t *i2c_obj){
    ...
    if(NULL == i2c_obj) {
        ...
        PIR1bits.SSPIF = 0; // Clear The MSSP Interrupt Flag bit -> SSPIF
        
        if(I2C_START_BIT_DETECTED == SSPSTATbits.S) { // Report The Start Condition Detection
...
STD_RETURN_TYPE MSSP_I2C_Master_Send_Repeated_Start(const mssp_i2c_t *i2c_obj) {
    ...
    if(NULL == i2c_obj) {
        ...
        while(SSPCON2bits.RSEN); // Wait for the completion of the Repeated Start condition
        PIR1bits.SSPIF = 0; // Clear The MSSP Interrupt Flag bit -> SSPIF
        ret = E_OK;
    }
    return ret;
...
STD_RETURN_TYPE MSSP_I2C_Master_Send_Stop(const mssp_i2c_t *i2c_obj) {
    ...
    if(NULL == i2c_obj) {
        ...
        while(SSPCON2bits.PEN); // Wait for the completion of the Stop condition
        PIR1bits.SSPIF = 0; // Clear The MSSP Interrupt Flag bit -> SSPIF
...
STD_RETURN_TYPE MSSP_I2C_Master_Write_Blocking(const mssp_i2c_t *i2c_obj, uint8 i2c_data, uint8 *_ack) {
    ...
    if((NULL == i2c_obj) || (NULL == _ack)) {
        ...
        SSPBUF = i2c_data; // Write Data to the Data register
        while(SSPSTATbits.BF); // Wait The transmission to be completed
        PIR1bits.SSPIF = 0; // Clear The MSSP Interrupt Flag bit -> SSPIF
...
STD_RETURN_TYPE MSSP_I2C_Master_Read_Blocking(const mssp_i2c_t *i2c_obj, uint8 ack, uint8 *i2c_data) {
    ...
    if((NULL == i2c_obj) || (NULL == i2c_data)) {
        ...
        I2C_MASTER_RECEIVE_ENABLE_CFG(); // Master Mode Receive Enable
        while(!(SSPSTATbits.BF)); // Wait for buffer full flag : A complete byte received
        *i2c_data = SSPBUF; // Copy The data registers to buffer variable
...
static inline void I2C_Master_Mode_Clock_Configurations(const mssp_i2c_t *i2c_obj) {
    SSPCON1bits.SSPM = i2c_obj->i2c_cfg.i2c_mode_cfg; // I2C Master mode, clock = FOSC/(4 * (SSPADD + 1))
    SSPADD = (uint8)(((_XTAL_FREQ / 4.0) / i2c_obj->i2c_clock) - 1);
}

static inline void I2C_Slave_Mode_Configurations(const mssp_i2c_t *i2c_obj) {
    SSPCON1bits.SSPM = i2c_obj->i2c_cfg.i2c_mode_cfg; // Four Bits
}
